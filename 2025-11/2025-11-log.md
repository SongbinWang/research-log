...
- 对项目中通用的`ResidualAttentionBlocks`的两种变体IVLP和MaPLe进行仔细学习，明白两种变体在代码上的实现是如何对每一层Transformer插入Prompt的，以及**两种变体设计的目的**：前者直接对两个分支插入独立的Prompt；后者两个模态的Prompt存在耦合关系

- 阅读论文：[Bi-modality Individual-aware Prompt tuning for Visual-Language Model](###BIP)

  BIP在TCP的基础上，为视觉分支也添加了类似的感知Prompt机制，使用相同的Adapter模块来生成Prompt并插入视觉编码器的中间层



**下阶段目标**：读BIP的代码，以及对BIP和以往的方法进行对比，进行消融实验
...
- 读[BIP](###BIP)的代码部分
  ...
  BIP是在TCP的基础上，在视觉端也采取了类似的操作，但是落实到代码实现上，**有许多论文中未提及的细节**

  1.关于promp learner部分, 设计了一个内置的分类器(全连接层), 训练过程中将数据向前传播了一遍，然后将参数给传递到这个线性层中。

  2.这个线性层还用于文中所述的视觉嵌入交叉熵损失计算，并且原文没有对这一项的权重做消融。在代码中这个权重被设置为0.5。 **针对这部分进行消融**

  3.将冻结编码器的输出特征与可学习编码器的输出特征进行了叠加。

- 对TCP和BIP的代码进行消融实验

  TCP使用dtd数据集重新进行消融测试

  对上面提及的这个线性层相关权重进行消融：对于代码中取0.5是为了权衡base和new类别的表现，从log上来看base和new分别权重成正比和反比。推断为权重越大偏移越往回拉，因此base表现也就变好

  对特征叠加部分进行消融



**下阶段目标**：继续对BIP进行消融测试，继续读论文(awt)
...
maple和PromptSRC在clip的设计上存在一定不同，在resblock的设计中，由于maple的图像端的Prompt由文本端经过映射而来，所以将每一层的Prompt合在一个tensor list里，经过每层的传播指定特定的Prompt。PromptSRC的文本和图像两段的Prompt是独立设计的，没有通过一个投影模块来进行耦合，因此代码中直接判断是否插入Prompt，是则进行构建插入。
...
在learnable Prompt的设计上和MaPLe一样，同样是将文本的Prompt给映射到了视觉端，来实现模态耦合
...
对插入Prompt的替换权重的消融： 原始权重1.0<img src="./assets/image-20251120144335360.png" alt="image-20251120144335360" style="zoom:50%;" />

设置为0.5<img src="./assets/image-20251120144413311.png" alt="image-20251120144413311" style="zoom:50%;" />
...
- 文本端的维度为512；视觉端为768。在设置模态对齐模块或者是其他插入模块时候需要注意

- 对可学习Prompt的一个全连接层进行了初始化

- 在BIP中，还将经过Prompt learner编码器的特征与原始特征进行了叠加


消融：原始<img src="./assets/image-20251120131137531.png" alt="image-20251120131137531" style="zoom:50%;" />

去除掉训练阶段的原始特征和可学习特征叠加的部分<img src="./assets/image-20251120111526892.png" alt="image-20251120111526892" style="zoom:50%;" />

将视觉嵌入的自交叉熵损失权重设置为0.2<img src="./assets/image-20251120125715048.png" alt="image-20251120125715048" style="zoom:50%;" />

将视觉嵌入的自交叉熵损失权重设置为0.7<img src="./assets/image-20251120131016702.png" alt="image-20251120131016702" style="zoom:50%;" />
...
| TCP              | 0.002  | 50    | 4     | 插入第8层    | 32         | adam  |                                                              |
| BIP              | 0.002  | 50    |       | 插入第8层    | 32         | adam  | 插入的感知Prompt长度：4                                      |
